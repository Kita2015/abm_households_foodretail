...
to go
  ...
  ask persons with [is-cook? = true]  [ ;cook collects food preferences of group
    ...
    let length-list length dinner-list

    (ifelse length-list > 1 [
      let chosen-item (random length-list) ;cooking skills of the cook are not considered here yet
      let chosen-meal item chosen-item dinner-list
      set meal-I-cooked chosen-meal   ;cook decides what type of meal to prepare
      set my-last-dinner chosen-meal
    ]
    length-list = 1 [
    let chosen-meal item 0 dinner-list ;go with the only diet preference on the dinner list - single household cook prepares his own preference
      set meal-I-cooked chosen-meal
      set my-last-dinner chosen-meal ;ERROR in some households no one is the cook / number of cooks changes per tick / some persons do not eat dinner
    ]
      ;do nothing
      )
  ...
  ask persons with [is-cook? = false] [
    let my-dinner-group persons with [h-id = [h-id] of myself] ;QUESTION how to combine the two conditions using AND
    let my-cook my-dinner-group with [is-cook? = true]

    ;was the food my preference?   ;did I like the food? if yes, confer cooking skills to cook

    set my-last-dinner [meal-I-cooked] of my-cook


  (ifelse my-last-dinner = diet [
    ask my-cook [set cooking-skills cooking-skills + 0.01]
    ]
    my-last-dinner != diet [
      ask my-cook [set cooking-skills cooking-skills - 0.01]
...
to update-meals-cooked
  let cooks persons with [is-cook? = true]
  print cooks
  let omnivore-cooks cooks with [meal-I-cooked = "omnivore"]
  print omnivore-cooks
  let omnivore-meals count omnivore-cooks
  print omnivore-meals


end


;to-report omnivore-cooked
;  let cooks persons with [is-cook? = true]
;  print cooks
;  let omnivore-cooks cooks with [meal-I-cooked = "omnivore"]
;  print omnivore-cooks
...
;  print omnivore-meals
;
;  report omnivore-meals
;
;end
;
;to-report pescatarian-cooked
;  let cooks persons with [is-cook? = true]
;  let pescatarian-cooks cooks with [meal-I-cooked = "pescatarian"]
;  let pescatarian-meals count pescatarian-cooks
;
;
;  report pescatarian-meals
;end
;
;to-report vegetarian-cooked
;  let cooks persons with [is-cook? = true]
;  let vegetarian-cooks cooks with [meal-I-cooked = "vegetarian"]
;  let vegetarian-meals count vegetarian-cooks
;
;  report vegetarian-meals
;end
;
;to-report vegan-cooked
;  let cooks persons with [is-cook? = true]
;  let vegan-cooks cooks with [meal-I-cooked = "vegan"]
;  let vegan-meals count vegan-cooks
;
;  report vegan-meals
;end
@#$#@#$#@
...
"" ""
PENS
"default" 1.0 0 -6459832 true "" "plot omnivore-cooked"
"pen-1" 1.0 0 -1664597 true "" "plot pescatarian-cooked"
"pen-2" 1.0 0 -4079321 true "" "plot vegetarian-cooked"
"pen-3" 1.0 0 -14439633 true "" "plot vegan-cooked"
