...
to setup-persons
  ask persons [
    ...
    set status-x random-float 1 + random-float (-1)
    set status-y (2 / ( 1 + e ^ ( (-1) * (0 - status-x))) - 1)
...
to select-meal ;person procedure
  ask persons with [is-cook? = true]  [
    ...
    (ifelse meal-selection = "status-based" [
      ...
      let vip-guest max-one-of my-dinner-guests [status-y] ;choose guest with highest status; this could in this model version also be himself
      let vip-meal "none"
      ask vip-guest [
  ...
  if meal-evaluation = "quality-based" [
    ...
    ask persons with [is-cook? = false][
      ...
      (ifelse last-meals-quality < 0.55 [
        ...
        ask my-cook [
          set status-x (status-x - 2)
          let new-status-y 2 / ( 1 + e ^ ( (-1) * (0 - status-x))) - 1
          set status-y new-status-y
        ]
        ]
...
to visualization
  ask persons [
    ...
    (ifelse status-y <= 0.25 [
      set size 1
      ]
      status-y > 0.25 and status-y <= 0.5 [
        set size 1.2
      ]
      status-y > 0.5 and status-y <= 0.75 [
        set size 1.4
      ]
      status-y > 0.75 and status-y <= 1 [
        set size 1.6
      ]
      ;if status exceeds 1
      [set size 1.8]
