...
XML=food2_7_params_spatial.xml
BDIR=.

printf -v EXPT_ID "spatial-%04d" $TASK_NO
RDIR="`pwd`/$BDIR"
...
cd "/mnt/apps/netlogo-6.2.2"
OUT="$RDIR/$EXPT_ID.out"
CSV="$RDIR/$EXPT_ID-table.csv"
HOME="$tmphome" $SHELL <<TMPHOMESH
srun  "/mnt/apps/netlogo-6.2.2/netlogo-headless-3gc-4Gi.sh" --model "$MDIR/2023-11-06 household_foodretail v 2_7.nlogo" --setup-file "$XDIR/$XML" --experiment "$EXPT_ID" --threads 1 --table "$CSV" > "$OUT" 2>&1
