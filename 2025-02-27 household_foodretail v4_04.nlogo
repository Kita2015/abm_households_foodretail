...
to check-restocking-tables
    ...
    if supply-demand = "dynamic-restocking" [
      ...
      if restocking-time = 0 and ticks != 0 [
        ...
        foreach diets-list [ diets ->
          ...
          ;table:put stock-table diets round ( current-stock + ( ( mean-sublist + (mean-sublist * 0.05) ) * restocking-frequency ) )
          table:put stock-table diets round  ( ( mean-sublist + (mean-sublist * 0.1) ) * restocking-frequency )
  ...
  ;check empty shelves
  ;in case all outlets had an empty shelve, an empty shelve for that particular protein source should be added in the empty shelves table

;  let nr-food-outlets length food-outlets-list
;
;  foreach table:keys food-outlets-table [ outlet ->
;    let sold-out-products table:get food-outlets-table outlet
;
;
;
;
